...
class CalendarController {
  ...
  DateTime _firstDayOfMonth(DateTime? month) {
    return DateTime.utc(month?.year ?? DateTime.now().year, month?.month ?? DateTime.now().month, 1, 12);
  }

  DateTime _lastDayOfMonth(DateTime? month) {
    final currentDate = month ?? DateTime.now();
    final date = currentDate.month < 12
        ? DateTime.utc(currentDate.year, currentDate.month + 1, 1, 12)
        : DateTime.utc(currentDate.year + 1, 1, 1, 12);
    return date.subtract(const Duration(days: 1));
  }

  DateTime _previousWeek(DateTime? week) {
    return (week ?? DateTime.now()).subtract(const Duration(days: 7));
  }

  DateTime _nextWeek(DateTime? week) {
    return (week ?? DateTime.now()).add(const Duration(days: 7));
  }

  DateTime _previousMonth(DateTime? month) {
    if (month?.month == 1) {
      return DateTime((month?.year ?? DateTime.now().year) - 1, 12);
    } else {
      return DateTime(month?.year ?? DateTime.now().year, (month?.month ?? DateTime.now().month) - 1);
    }
  }

  DateTime _nextMonth(DateTime? month) {
    if (month?.month == 12) {
      return DateTime((month?.year ?? DateTime.now().year) + 1, 1);
    } else {
      return DateTime(month?.year ?? DateTime.now().year, (month?.month ?? DateTime.now().month) + 1);
    }
  }
  ...
  bool _isExtraDayBefore(DateTime day) {
    return day.month < (_focusedDay ?? DateTime.now()).month;
  }
